...
`POST /admin/questions` accepts `{ "questions": [...] }` (up to 50 items) and requires an `x-admin-api-key` header. The frontend exposes this safely through `/admin`: it validates an administrator password server-side, then proxies the request with the backend key. Neither key is bundled for the browser.
...
`acceptedAnswers`, `explanation`, and `active` are optional. New immutable IDs are generated by the API.
